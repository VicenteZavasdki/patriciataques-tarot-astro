...
- Name: pages/index.spec.ts >> Landing Page >> pricing section shows both currencies
- Location: tests/e2e/pages/index.spec.ts:42:3
...
Locator: locator('text=Brasil')
Expected: visible
Error: strict mode violation: locator('text=Brasil') resolved to 4 elements:
    1) <div class="text-sm text-muted-foreground">São Paulo, Brasil</div> aka getByText('São Paulo, Brasil')
    2) <div class="absolute top-0 right-0 bg-gold-500 text-background text-xs font-bold px-4 py-1.5 rounded-bl-lg">↵Brasil↵</div> aka getByText('Brasil', { exact: true })
    3) <div class="font-semibold text-foreground mb-2">Leitura Brasil</div> aka getByRole('radio', { name: 'R$ 260,00 Leitura Brasil Sess' })
    4) <option value="BR">Brasil 🇧🇷</option> aka getByLabel('País *')

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for locator('text=Brasil')
